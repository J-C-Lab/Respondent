...
本子项目的默认实现目标是 **Windows 10 及以上**。原因是事件驱动 loopback 在旧 Windows 版本上存在兼容性差异：旧版本可能初始化成功但不会触发 loopback capture event。MVP 不实现旧系统 fallback；如果运行环境低于 Windows 10，采集启动应返回 `Unsupported("event-driven loopback requires Windows 10 or later")` 或等价错误。后续如需兼容旧系统，再单独设计 timer/polling fallback。
...
- `capture.rs`（重写）：`cfg(windows)` 的 WASAPI loopback 采集线程与生命周期；非 Windows 返回明确 unsupported 错误，避免假装采集成功。
...
2. **重采样（有状态）**：
   ...
   ```
   线性插值。`pos` 表示当前输出采样落在“上一帧最后一个样本 + 当前输入”的拼接序列上的小数位置，`step = src_rate / dst_rate`。每次 `process` 时，如果存在 `last`，先把 `last` 作为索引 0 的前置样本；输出循环只在 `pos + 1 < extended.len()` 时插值，避免越界猜测未来样本。处理完成后：
   - `last = input.last()`。
   - `pos -= input.len()`，保留跨块小数位置。
   - 如果 `pos < 0.0`，归零，避免浮点误差累积。

   数值例子：48k→16k 时 `step = 3.0`。第一块 `[0, 1, 2, 3]` 输出落点为 `0`，保留 `last = 3`、`pos = 0`；第二块 `[4, 5, 6, 7]` 的扩展序列为 `[3, 4, 5, 6, 7]`，下一输出从前一块边界后的正确位置继续，不重置到 `4`。跨帧测试必须比较“一次性处理完整输入”和“拆成两块处理后拼接”的输出，边界附近不得出现重复或跳变。
...
4. `GetMixFormat` 得 `WAVEFORMATEX`/`WAVEFORMATEXTENSIBLE`：`src_rate`、`channels`、格式标签、subformat、位深。
...
7. 循环直到停止信号：`WaitForSingleObject(event, timeout)` → `GetBuffer` → 按格式把字节解释为交错 f32（支持格式见下文）→ `pipeline.push_interleaved_f32(...)` → 帧入 channel → `ReleaseBuffer`。`AUDCLNT_BUFFERFLAGS_SILENT` 标记的包按静音处理。
8. 退出前 `Stop`、释放 COM。

### 关键决策

- **背压**：不要在 `crossbeam_channel::Receiver` 上读取来“丢最旧帧”，因为 `receiver()` 会暴露给消费者，内部抢读会破坏消费语义。MVP 采用更简单稳定的策略：channel `bounded(128)`，使用 `try_send`；满时**丢弃当前新帧**并递增 `dropped_frames: AtomicU64`。这会造成 ASR 少量缺帧，但不会阻塞 WASAPI 循环，也不会偷走消费者尚未读取的旧帧。后续如果需要“丢最旧帧”，应改用单生产者内部 ring buffer/`ArrayQueue`，再由一个唯一转发任务写入消费者 channel。
...
- `WAVE_FORMAT_PCM`，16-bit signed integer。
- `WAVE_FORMAT_EXTENSIBLE` + `KSDATAFORMAT_SUBTYPE_IEEE_FLOAT`，32-bit float。
- `WAVE_FORMAT_EXTENSIBLE` + `KSDATAFORMAT_SUBTYPE_PCM`，16-bit signed integer。

其他格式（例如 PCM24、PCM32、非整数位深、未知 subformat）在 MVP 中返回 `CaptureError::Unsupported(...)`。不要静默解释未知格式。

### CaptureError

`thiserror` 定义：`Com(windows::core::Error)`、`NoDefaultEndpoint`、`Unsupported(String)`（不支持的 OS、格式标签、subformat 或位深）、`ThreadJoin(String)`。
...
- 格式解析：float32、PCM16、extensible float32、extensible PCM16 被接受；PCM24/未知 subformat 返回 `Unsupported`。
...
- Windows 版本验证：Windows 10+ 路径可启动；低于 Windows 10 时返回 unsupported，不等待永远不会触发的 loopback event。
...
- channel 满时不会阻塞 WASAPI 线程；新帧被丢弃并累计 dropped frame 计数。
